<div class="card rounded">
    <ul class="list-group p-3">
        <li class="list-group-item bg-transparent border-bottom">
            <span class="float-start">Subtotal</span>
            <span class="float-end">$ 0</span>
        </li>
        <li class="list-group-item bg-transparent border-bottom">
            <span class="float-start">Descuento</span>
            <span class="float-end">$ 0</span>
        </li>
        <li class="list-group-item bg-transparent border-bottom">
            <span class="float-start">Impuesto</span>
            <span class="float-end">$ 0</span>
        </li>
    </ul>
</div>

<h6 class="text-center px-3 py-4 my-3 rounded border-0 w-100 font-weight-bold <?php if (empty($order)): ?> bg-light <?php else: ?> backColor <?php endif ?>" id="granTotal">Gran Total: $ 0</h6>